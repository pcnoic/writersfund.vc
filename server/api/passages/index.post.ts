...
import { buildAiStoryFromNarrative, getWordCount, summarizeNarrative } from '~/server/utils/narrative'
...
export default defineEventHandler(async (event) => {
  ...
  if (!body.title || !body.content || !body.genre) {
    throw createError({ statusCode: 400, statusMessage: 'title, content and genre are required.' })
  }

  if (body.content.length < 120) {
    throw createError({ statusCode: 400, statusMessage: 'Passage content must be at least 120 characters.' })
  }
  ...
  const { error: insertPassagesError } = await supabase.from('passages').insert([
    {
      id: writerPassageId,
      user_id: user.id,
      kind: 'writer',
  ...
  if (insertPassagesError) {
    throw createError({ statusCode: 500, statusMessage: insertPassagesError.message })
  }
