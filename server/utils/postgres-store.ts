...
export async function readStore(): Promise<Store> {
  ...
  const [dbUsers, dbChallenges, dbSessions, dbPassages, dbMatchups, dbBallots, dbVotes, dbTournaments, dbApplications] =
    await Promise.all([
...
export async function writeStore(_store?: Store): Promise<void> {
  throw createError({
    statusCode: 500,
    statusMessage: 'writeStore is unavailable with Postgres mode. Use explicit mutations.'
  })
}
...
export async function getUserBySession(sessionId: string): Promise<User | null> {
  ...
  const session = await db.query.sessions.findFirst({
    where: (table, { and, eq, gt }) => and(eq(table.id, sessionId), gt(table.expiresAt, new Date()))
  })
...
export async function getOrCreateBallot(voterId: string): Promise<{
  ...
  const candidateMatchups = await db
    .select()
    .from(matchups)
    .where(and(eq(matchups.status, 'open'), lte(matchups.opensAt, now), gt(matchups.closesAt, now)))
    .orderBy(asc(matchups.opensAt))

  for (const matchup of candidateMatchups) {
    const vote = await db.query.votes.findFirst({
      where: (table, { and, eq }) => and(eq(table.matchupId, matchup.id), eq(table.voterId, voterId))
    })
...
export async function castVote(input: {
  ...
  const alreadyVoted = await db.query.votes.findFirst({
    where: (table, { and, eq }) => and(eq(table.voterId, input.voterId), eq(table.matchupId, ballot.matchupId))
  })
...
export async function getMatchupResult(matchupId: string): Promise<{
  ...
  const matchupVotes = await db.select().from(votes).where(eq(votes.matchupId, matchup.id))
