...
import { buildAiStoryFromNarrative, getWordCount, summarizeNarrative } from './narrative'
...
async function writeFileStore(store: Store): Promise<void> {
  await fs.writeFile(STORE_FILE, JSON.stringify(normalizeStore(store), null, 2), 'utf8')
}
...
function openMatchupsForVoter(store: Store, voterId: string): Matchup[] {
  const alreadyVoted = new Set(
    store.votes.filter((vote) => vote.voterId === voterId).map((vote) => vote.matchupId)
  )
  ...
  return store.matchups.filter((matchup) => {
    ...
    const now = Date.now()
    return now >= Date.parse(matchup.opensAt) && now <= Date.parse(matchup.closesAt)
  })
}
...
function computeLeaderboard(store: Store): LeaderboardEntry[] {
  ...
  const sortedVotes = [...store.votes].sort((a, b) => a.createdAt.localeCompare(b.createdAt))

  for (const vote of sortedVotes) {
    const matchup = store.matchups.find((item) => item.id === vote.matchupId)
    if (!matchup) continue

    const writerPassage = store.passages.find((item) => item.id === matchup.writerPassageId)
...
export async function upsertUserProfile(input: {
  ...
  let user = store.users.find((item) => item.email.toLowerCase() === normalizedEmail)
  if (!user) {
    user = {
      id: `u-${randomUUID()}`,
...
export async function verifyAuthChallenge(email: string, code: string): Promise<{
  session: Session
  user: User
}> {
  if (usePostgres) {
  ...
  if (!challenge) {
    throw createError({ statusCode: 401, statusMessage: 'Invalid or expired login code.' })
  }
...
export async function getUserBySession(sessionId: string): Promise<User | null> {
  ...
  store.sessions = store.sessions.filter((session) => !isExpired(session.expiresAt))
...
export async function createPassage(input: {
  ...
  content: string
  genre: string
}): Promise<{ passage: Passage; aiPassage: Passage; matchup: Matchup; narrative: string; wordCount: number }> {
  if (usePostgres) {
  ...
  if (wordCount > 3000) {
    throw createError({ statusCode: 400, statusMessage: 'Passage must be 3000 words or fewer.' })
  }
  ...
  const activeTournament = store.tournaments.find((item) => item.status === 'active')
  if (!activeTournament) {
    throw createError({ statusCode: 500, statusMessage: 'No active tournament configured.' })
  }
...
export async function getOrCreateBallot(voterId: string): Promise<{
  ...
  const writerPassage = store.passages.find((passage) => passage.id === matchup.writerPassageId) ?? null
  const aiPassage = store.passages.find((passage) => passage.id === matchup.aiPassageId) ?? null
...
export async function castVote(input: {
  ...
  const alreadyVoted = store.votes.find(
    (vote) => vote.voterId === input.voterId && vote.matchupId === ballot.matchupId
  )
  if (alreadyVoted) {
    throw createError({ statusCode: 409, statusMessage: 'You have already voted on this matchup.' })
  }

  const winnerPassageId = input.choice === 'A' ? ballot.optionA : ballot.optionB
  const vote: Vote = {
    id: `v-${randomUUID()}`,
...
export async function buildLeaderboard(store?: Store): Promise<LeaderboardEntry[]> {
  if (usePostgres) {
...
export async function getMatchupResult(matchupId: string): Promise<{
  ...
  for (const vote of store.votes.filter((item) => item.matchupId === matchup.id)) {
